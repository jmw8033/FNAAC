...
/* Release metadata is deliberately independent from feature edits. The
...
const GRAPH_B = {
  ...
  b24:{ x: 52.9, y: 19.7, edges:["b27","b31"] },               // CORRIDOR E
  b25:{ x: 87.9, y:  9.4, edges:[] },                          // SALES 1
  b26:{ x: 68.8, y:  9.4, edges:[] },                          // SALES 2
  b27:{ x: 48.4, y:  9.4, edges:[] },                          // SALES 3
  b28:{ x: 21.0, y: 24.1, edges:["b31","b29"] },              // ATRIUM N
  b29:{ x: 32.1, y:  8.1, edges:["b30","b31"] },              // ATRIUM S
  b31:{ x: 36.6, y: 19.4, edges:["b28","b29","b24"] },       // ATRIUM DOOR APPROACH
...
const DOOR_LINKS = [
  ...
  { id:"D5", name:"PAYROLL DOOR", a:"b17", b:"b18", pin:{x:36,y:43.8}, oneShot:true, breakMs:2800 },
  { id:"D6", name:"ATRIUM DOOR", a:"b24", b:"b31", pin:{x:38.8,y:19.5} },
...
const CAMERAS_B = {
  ...
  C24:{ name:"9D  ATRIUM", floor:2, pin:{x:8,y:26.5}, pan:{width:114,period:34000,phase:4.8}, sees:["b28","b29","b30","b31"], art:"images/cameras/C24.jpg" }
};
...
/* ---------------------------------------------------------------------------
   ...
   These are deliberately just image slots plus placement. Drop transparent
   PNGs at the paths below and they appear on the office desk after the matching
   challenge is completed. The calibration tool edits left/bottom/width/rotate
...
const CONFIG = {
  ...
  botDwellMs: 1200,        // ordinary search-camera dwell
  botHuntMs:  7500,        // how long a Eugene flicker keeps it actively searching
  botHuntCooldownMs: 8000, // quiet period after a hunt, so flickers cannot chain
  botHallCheckMs: 7000,    // occasional doorway-camera glance
  botDoorCheckMs: 750,     // monitor DOWN, eyes on the office after a Eugene flicker
  botCamLow: 34,           // below this, bank a recharge unless something is urgent
  botCamResume: 92,        // three-second recharge is cheap; come back nearly full
  ...
  walkByStaticMs: 900,     // camera snow from that crossing; animation itself stays 2.6s
...
const SECRET_COMPLETE_KEY = "fnaac-midnight-shift-complete";

/* SECRET CUSTOM NIGHT — exact A.I. combination, editable in calibrate.html.
   The default 1 / 9 / 8 / 7 is intentionally obscure enough to behave like an
   easter egg. The selected values only unlock the mode; once it starts, the
   ordinary four-character Custom Night roster is replaced by three Sloppys.

   moveMs is the direct movement-opportunity interval by in-game hour. Each
   clone carries a tiny rhythm offset and fresh ±10% jitter so they naturally
   drift instead of becoming a single impossible five-second metronome. */
const SECRET_NIGHT = {
...
function loadCustomLevels(){
  try {
    ...
    secretNightComplete = localStorage.getItem(SECRET_COMPLETE_KEY) === "1";
  } catch (_) {}
}
...
function saveSecretNightComplete(){
  secretNightComplete = true;
  try { localStorage.setItem(SECRET_COMPLETE_KEY, "1"); } catch (_) {}
}

function secretComboMatched(levels = customLevels){
  return Object.keys(SECRET_NIGHT.combo).every(id =>
    Number(levels[id]) === Number(SECRET_NIGHT.combo[id]));
}
function secretNightMode(){
  return S.night === 8 && secretComboMatched(customLevels);
}

function secretSloppyArt(i){
  const list = SECRET_NIGHT.sloppyArt || [];
  return list[Math.max(0, Math.min(2, Number(i) || 0))] || "";
}
function secretSloppyEyeColor(i){
  const list = SECRET_NIGHT.eyeColors || [];
  return list[Math.max(0, Math.min(2, Number(i) || 0))] || "#9ec8ff";
}
function secretSloppyPlaceholder(i){
  const c = secretSloppyEyeColor(i);
  const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 700">' +
    '<path fill="#080b0c" d="M210 70c-74 0-118 54-118 126v54C38 288 20 377 28 700h364c8-323-10-412-64-450v-54C328 124 284 70 210 70z"/>' +
    '<circle cx="170" cy="188" r="18" fill="'+c+'"/><circle cx="250" cy="188" r="18" fill="'+c+'"/>' +
    '</svg>';
  return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
}
function setSecretSloppyImage(img, cloneIndex){
  if (!img) return;
  const wanted = secretSloppyArt(cloneIndex);
  img.onerror = () => { img.onerror = null; img.src = secretSloppyPlaceholder(cloneIndex); };
  img.src = wanted || secretSloppyPlaceholder(cloneIndex);
}
...
function authoredSettingNightIndex(){
  if (secretNightMode()) return Math.max(0, Math.min(6, SECRET_NIGHT.settingNight - 1));
...
const FATIGUE = {
  fullMs: 9000,   // retained for the optional autopilot logic; no longer used by live character movement
  floor: 0.28,
  recover: 1.7
};
...
const SLOPPY = {
  ...
  /* REQUIRED WATCH TIME. Five seconds on every night and every Custom AI
     level. This is accumulated attention, not continuous staring: switching
     cameras does not erase it. It resets only when her next movement opportunity
     consumes the check. */
  watchMs:  [5000, 5000, 5000, 5000, 5000, 5000, 5000],
...
function sloppyCfg(){
  ...
  if (S.night === 8){
    const d = customLevel("sloppy");
    const t = customScale(d);
    const authoredBase = customPairCfg("sloppy").moveMs;
    return {
      ...
      /* The benchmark exposed a second constraint beyond "five seconds must
         fit": at AI 20 Gordon also asks for short camera searches between
         Sloppy checks. A 6.3s absolute-minimum cycle left only ~1.3s for every
         other mechanic combined, so perfect input could still lose to schedule
         overlap. Authored levels 1-17 remain untouched. The extrapolated 18-20
         range gets a 4.5s base floor; AI 20 still runs faster than Night 7
         overall because moveMult keeps extrapolating, but its worst 5 AM cycle
         is about 8.9s, leaving ~3.9s for Gordon/Jeffrey/maintenance. */
      moveBaseMs: d > 17 ? Math.max(4500, authoredBase) : authoredBase,
      watchMs: 5000
    };
  }
  const h = nightIdx();
  return { doorMs:SLOPPY.doorMs[h], moveMult:SLOPPY.moveMult[h],
           moveBaseMs:NIGHTS[h].moveMs, watchMs:SLOPPY.watchMs[h] };
}
...
const PAIRS = {
  ...
  soundAndSight: [
    { id:"eugene",  name:"EUGENE",  lureResist:0.05, watchStall:0, passByMod:+0.05,
    ...
    { id:"sloppy", name:"SLOPPY", lureResist:0.20, watchStall:0, passByMod:-0.05, speedMod:0.95,
      doubleStepMod:-1,
...
const GORDON = {
  ...
  artPurple: asset("images/characters/ani_gordon_purple.png"),
  artRed:    asset("images/characters/ani_gordon_red.png"),
  ...
  /* THE TELL.
     Gordon warns before every walking step. The lead is re-rolled each cycle
     around four seconds so it cannot become a metronome: normally 3.55-4.45s.
     On an extremely fast Custom-20 upstairs cycle it is clamped just inside the
     actual move interval, so the warning can never belong to the previous step. */
  tellMs: 4000,
  tellJitterMs: 450,
...
const POWER = {
  ...
  /* THESE FOUR ARE NOT GUESSES. They were solved against a stated design — how
     ...
     monitor 40%, one door 86%, and 1.38% per cue. Which is the shape you want
  ...
  drain: {
    ...
    monitor:0.140,   // the monitor raised. One usage bar; camera battery limits long holds.
...
const FAULTS = {
  ...
  ventDark: {
    ...
    graceMs: 7000,
    /* The grace is now truly safe. A player who cycles the lights before this
       timer expires is using the system correctly and cannot lose ventilation
       to a low-probability roll anyway. Past the grace, the normal wear ramp
...
function chooseDuctAny(from, shutId){
  const d = distFromBlocked(from, shutId);
  const opts = DUCTS.filter(v => d[v.down] !== undefined);
...
const sloppyReadyGrowlFx = () => {
  ...
  f.type = "lowpass"; f.frequency.value = 300; f.Q.value = 1.0;
  o1.type = "sawtooth"; o1.frequency.setValueAtTime(118, t);
  o1.frequency.exponentialRampToValueAtTime(66, t + .52);
  o2.type = "triangle"; o2.frequency.setValueAtTime(236, t);
  o2.frequency.exponentialRampToValueAtTime(132, t + .52);
  g1.gain.setValueAtTime(.0001, t);
  g1.gain.exponentialRampToValueAtTime(.105, t + .04);
  g1.gain.exponentialRampToValueAtTime(.0001, t + .58);
  g2.gain.setValueAtTime(.0001, t);
  g2.gain.exponentialRampToValueAtTime(.038, t + .045);
  g2.gain.exponentialRampToValueAtTime(.0001, t + .55);
  o1.connect(f).connect(g1).connect(dst);
  o2.connect(f).connect(g2).connect(dst);
  o1.start(t); o2.start(t);
  o1.stop(t + .62); o2.stop(t + .60);
  noise(.42, "lowpass", 190, .050, undefined, x => Math.sin(x * Math.PI));
};

/* Sloppy tried to take the movement you just prevented. This is intentionally
   louder and sharper than the five-second "ready" growl: hearing it means that
   movement opportunity has been consumed and her watch bank has reset to zero.
   It is the player's cue to start building the next five seconds. */
const sloppyBlockedFx = () => {
  if (!actx) return;
  tone(132, .22, "sawtooth", .16, 74);
  tone(264, .18, "triangle", .055, 142);
  setTimeout(() => tone(104, .30, "sawtooth", .14, 58), 145);
  setTimeout(() => noise(.30, "lowpass", 260, .11, undefined,
    x => Math.pow(Math.sin(x * Math.PI), 1.35)), 55);
};

/* Low enough to sound ugly and threatening, but with an audible harmonic so
   laptop/phone speakers do not lose the entire laugh below their cutoff. */
const all20LaughFx = () => {
  if (!actx) return;
  const beats = [
    {f:82, d:.26, v:.22, to:58},
    {f:72, d:.23, v:.24, to:52},
    {f:78, d:.29, v:.23, to:55},
    {f:64, d:.40, v:.26, to:45}
  ];
  let at = 0;
  beats.forEach((b, i) => {
    setTimeout(() => {
      tone(b.f, b.d, "sine", b.v, b.to);
      tone(b.f * 2, b.d * .92, "triangle", b.v * .30, b.to * 2);
      tone(b.f * 3, b.d * .68, "sine", b.v * .07, b.to * 3);
      noise(.18 + i * .02, "lowpass", 240, .065, undefined, t =>
        Math.pow(Math.sin(t * Math.PI), 1.6));
    }, at);
    at += 120 + i * 55;
  });
  setTimeout(() => {
    tone(46, .62, "triangle", .12, 32);
    tone(92, .55, "sine", .028, 64);
  }, 330);
};
...
const SFX = {
  ...
  sloppyReady:   { src:null, vol:0.42, synth:() => sloppyReadyGrowlFx() },
  sloppyBlocked: { src:null, vol:0.80, synth:() => sloppyBlockedFx() },
  all20Laugh: { src:null, vol:0.84, synth:() => all20LaughFx() },
...
/* Exact sound-engine hook used by sound_test.html. It intentionally exposes
   only read-only metadata plus a play button; it cannot alter game state. */
window.FNAAC_SOUND_TEST = {
  names: Object.keys(SFX),
  source(name){ return SFX[name] ? (SFX[name].src || null) : null; },
  play(name){
    if(!SFX[name]) return false;
    /* When called from sound_test.html, silence the hidden game's own title /
       room ambience first so the selected cue can be judged in isolation. */
    try { titleScreenOff(); stopRoomTone(); setHiss(false); setVentHum(false); } catch (_) {}
    if(!actx) initAudio();
...
[["concrete", ["b01","b02","b03","b04","b05","b12","b13"]],
 ["tile",     ["b15","b20","b21","b22","b23","b24","b28","b29","b31"]]
...
const S = {
  ...
  cameraStability:100,      // 0..100; rendered as five camera-battery cells
  ...
  failAcc:0, lastFrame:0, raf:null, breathAcc:0, flicker:0,
  sloppyConfirmFlicker:0, sloppyResetPulse:0, ambAcc:0,
...
function applyBuildMeta(){
  const v = $("titleVersion"), u = $("titleUpdated");
  if (v) v.textContent = "V " + (BUILD_META.version || "");
  if (u){
    const txt = String(BUILD_META.updated || "").trim();
    u.textContent = txt ? "LAST UPDATED  " + txt : "";
    u.hidden = !txt;
  }
}
...
function deskRewardUnlocked(id){
  if (id === "night5") return completedNight >= 5;
  if (id === "night7") return completedNight >= 7;
  if (id === "midnight") return secretNightComplete;
  if (id === "all20") return custom20Complete;
  return false;
}

function renderDeskRewards(){
  const host = $("deskRewards");
  if (!host) return;
  host.innerHTML = "";
  const fallbackName = { night5:"N5", night7:"N7", midnight:"MID", all20:"20" };
  Object.entries(DESK_REWARDS).forEach(([id, cfg]) => {
    if (!deskRewardUnlocked(id) || !cfg || !cfg.src) return;
    const place = node => {
      node.dataset.reward = id;
      node.style.left = (cfg.left || 0) + "%";
      node.style.bottom = (cfg.bottom || 0) + "%";
      node.style.width = (cfg.width || 6) + "%";
      node.style.transform = "translateX(-50%) rotate(" + (cfg.rotate || 0) + "deg)";
    };
    const fallback = document.createElement("div");
    fallback.className = "deskRewardFallback";
    fallback.textContent = fallbackName[id] || id.toUpperCase();
    place(fallback);
    host.appendChild(fallback);

    const img = document.createElement("img");
    img.className = "deskReward"; img.alt = ""; place(img);
    /* The placeholder remains until the real file proves it can load. This
       makes reward unlocks visible immediately while the art folder is still
       being populated, without ever exposing a broken-image icon. */
...
function startNight(n){
  ...
  const secret = secretNightMode();
  trackGameEvent("night_start", { night_number:n, secret_night:secret });
  document.body.classList.toggle("secret-night", secret);
  ...
  S.sloppyConfirmFlicker = 0; S.sloppyResetPulse = 0;
  ...
  S.mapFloor = 1;
  S.lastCam = {};
  S.gordon = secret ? null : spawnGordon();
  ...
  const makeRoamer = (a, node, i, special=false) => ({
    ...
    art:special ? secretSloppyArt(i) : a.art,
    /* Secret-night clones are deliberately close but not identical clocks.
       Normal roamers keep the authored 1 : 1.27 irrational-ish split. */
  ...
  if (secret){
    const sloppy = ROSTER.find(a => a.commitModel);
    const entrances = ["gW","gE","gS"];
    S.units = entrances.map((node,i) => makeRoamer(sloppy,node,i,true));
  } else {
    ...
    S.units = ROSTER.slice(0, 2).filter(a => customEnabled(a.id))
      .map((a, i) => makeRoamer(a, spots[i % spots.length], i, false));
  }
  ...
  applyDoorVars();
  el.nightLabel.textContent = secret ? SECRET_NIGHT.label : (n === 8 ? "CUSTOM NIGHT" : "NIGHT " + n);
  renderDeskRewards();
  syncChrome();
...
function playCue(){
  ...
  const targetNodes = [...(CAMERAS[S.activeCam].sees || [])];
  const pending = {
    nodes: targetNodes,
    strength: CONFIG.lureStrength,
    cam: S.activeCam,
    canRecallSloppy: true
  };
  S.units.forEach(u => {
    /* Normal audio is deferred until the next movement opportunity. Sloppy's
       doorway emergency is the one exception: once she is already breaching
       there will never be another movement check, so preserve the intended
       one-shot recall roll here. */
    if (u.commitModel && u.breaching){
      lureSloppyFromDoor(u, pending);
      return;
    }
    if (!u.lureMoves) u.pendingLure = pending;
  });
  S.lure = pending;
...
function toggleLastDoor(){
  ...
  /* Releasing a door is fail-open and therefore never needs the mains. This
     matters during the exact overlap the lower floor is designed to create:
     Eugene can force the office dark while Gordon is being blocked. Once the
     impact has happened the player must be able to stop paying for that door
     without briefly relighting the office. Closing/re-closing still needs both
     power and a healthy array. */
  if (S.doorShut === id){
    S.doorReadyAt = performance.now() + CONFIG.doorCooldownMs;
    S.doorShut = null; play("doorOpen");
    return;
  }
  if (!S.power){ buzz(); return; }
  if (!S.sys.door.ok){ buzz(); return; }
  S.doorReadyAt = performance.now() + CONFIG.doorCooldownMs;
  S.doorShut = id; play("doorShut");
}
...
/* GORDON'S EYES FOLLOW THE DIFFICULTY TIER.

   Base       Nights 1-5 / Custom tiers 1-5
   Purple     Nights 6-7 / Custom totals that map to Night 6 or Night 7
   Red        20/20/20/20 only

   The two variant files are optional while developing. Every DOM use installs
   a fallback to the ordinary sprite, so a missing new PNG never turns Gordon
   into a broken-image icon. */
function gordonTheme(customMenu){
  const custom = customMenu || S.night === 8;
  if (custom && customAllAt(20)) return "red";
  const idx = custom ? customSettingNightIndex() : nightIdx();
  return idx >= 5 ? "purple" : "base";
}
function gordonArtForTheme(theme){
  return theme === "red" ? GORDON.artRed : (theme === "purple" ? GORDON.artPurple : GORDON.art);
}
function gordonArtForCurrentDifficulty(){ return gordonArtForTheme(gordonTheme(false)); }
function gordonArtForCustomMenu(){ return gordonArtForTheme(gordonTheme(true)); }
function setGordonImage(img, src){
  if (!img) return;
  const wanted = src || GORDON.art;
  img.onerror = wanted !== GORDON.art ? () => { img.onerror = null; img.src = GORDON.art; } : null;
  if (img.getAttribute("src") !== wanted) img.src = wanted;
}
function setUnitImage(img, u){
  if (!img || !u) return;
  if (u.specialSloppy){
    const wanted = u.art || secretSloppyArt(u.cloneIndex);
    if (img.getAttribute("src") !== wanted){
      img.onerror = () => { img.onerror = null; img.src = secretSloppyPlaceholder(u.cloneIndex); };
      img.src = wanted || secretSloppyPlaceholder(u.cloneIndex);
    }
    return;
  }
  if (u.id === GORDON.id) setGordonImage(img, u.art || gordonArtForCurrentDifficulty());
  else if (img.getAttribute("src") !== u.art) img.src = u.art;
}
function updateGordonTitleFace(){
  /* The stylesheet always keeps base Gordon as a second background layer, so
     the themed asset can be selected synchronously and still fail safely.
     This also removes the async Image-probe race that could leave the ordinary
     sprite showing after the player had already selected Night 6/7. */
  document.documentElement.style.setProperty("--titleFace", 'url("' + gordonArtForCurrentDifficulty() + '")');
}

function updateGordonCustomFace(){
  document.documentElement.style.setProperty("--customFace", 'url("' + gordonArtForCustomMenu() + '")');
}
...
function buildCustomMenu(){
  ...
  const defs = [
    ...
    { id:"gordon",  name:"GORDON",  art:gordonArtForCustomMenu() },
...
function refreshCustomMenu(){
  ...
  host.querySelectorAll('.customCard').forEach(card => {
    ...
    if (id === "gordon") setGordonImage(card.querySelector('.customArt'), gordonArtForCustomMenu());
  ...
  updateGordonTitleFace();
  updateGordonCustomFace();
  const total = customTotalLevel();
...
function startCustomNight(){
  ...
  saveCustomLevels();
  S.night = 8;
  /* READY changes the title from the Night-7 menu state into an actual Custom
     Night. Refresh Gordon immediately so All-20 can become red during the
     handoff instead of inheriting whatever the previous title selection used. */
  updateGordonTitleFace();
  $("customScreen").classList.remove("show");
  document.body.classList.remove("custom-menu");
  el.title.classList.add("show");
...
function markStars(){
  /* Fixed achievement order, regardless of the order the player completes the
     two Custom Night challenges: GOLD (N5), PURPLE (N7), BLUE (Midnight), RED
     (20/20/20/20). Thus unlocking Midnight after All-20 inserts its blue star
     before the existing red one instead of moving red away from the end. */
  const earned = [
    [completedNight >= 5, "gold",   "Night 5"],
    [completedNight >= 7, "purple", "Night 7"],
    [secretNightComplete, "blue",  SECRET_NIGHT.label],
    [custom20Complete, "red",      "20/20/20/20"]
  ];
  STAR_DISPLAYS.forEach(id => {
    const host=$(id); if(!host) return;
    host.innerHTML=earned.filter(x=>x[0]).map(([,color,label]) =>
      '<span class="achievementStar star-'+color+'" title="'+label+'">★</span>'
    ).join('');
  });
}
...
function scheduleAll20TitleLaugh(){
  ...
  all20TitleLaugh = setTimeout(() => {
    if (!titleAlive || !all20Mode()) return;
    whenAudioRunning(() => play("all20Laugh", { vol:0.72 + Math.random() * 0.16, pan:Math.random() * .8 - .4 }));
...
function prepareSecretStartTrio(){
  const trio=$("secretPreNightTrio");
  if(!trio) return;
  const cfgs=SECRET_NIGHT.startTrio || [];
  trio.querySelectorAll(".secretStartClone").forEach((box,i)=>{
    const cfg=cfgs[i] || {};
    box.style.left=(cfg.left ?? [31,50,69][i])+"%";
    box.style.top=(cfg.top ?? [16,10,16][i])+"%";
    box.style.width=(cfg.width ?? [28,35,28][i])+"%";
    box.style.zIndex=String(i===1?4:3);
    const img=box.querySelector("img");
    setSecretSloppyImage(img,i);
...
function flashSecretStartEyes(){
  const trio=$("secretPreNightTrio"); if(!trio) return;
  trio.querySelectorAll(".secretCloneEyes").forEach((eyes,i)=>{
    eyes.classList.remove("flash"); void eyes.offsetWidth;
    setTimeout(()=>eyes.classList.add("flash"),i*45);
  });
}
function clearSecretStartEyes(){
  const trio=$("secretPreNightTrio"); if(!trio) return;
  trio.querySelectorAll(".secretCloneEyes").forEach(x=>x.classList.remove("flash"));
}
...
function showPreNight(){
  ...
  const secret = secretNightMode();
  updateGordonTitleFace();
  if (secret) prepareSecretStartTrio();
  clearSecretStartEyes();
  if (night) night.textContent = secret ? SECRET_NIGHT.label : (S.night === 8 ? "CUSTOM NIGHT" : "NIGHT " + S.night);
  title.classList.toggle("all20-preNight", typeof all20Mode === "function" && all20Mode());
  title.classList.toggle("secret-preNight", secret);
  title.classList.toggle("purple-preNight", !secret && gordonTheme(false) === "purple");
  ...
  titleScreenOn();
  /* Make the special laugh unmistakable during the all-20 handoff instead of
     relying only on the title scheduler's random delay. */
  if (all20Mode()) whenAudioRunning(() => play("all20Laugh", { vol:0.92, pan:Math.random()*.6-.3 }));
  ...
  preNightTimer = setTimeout(() => {
    ...
    if (secret) flashSecretStartEyes();
    else if (eyes){
    ...
    preNightTimer = setTimeout(() => {
      ...
      initAudio();
      startNight(S.night);
      title.classList.remove("preNight", "secret-preNight");
...
/* Roaming bodies do not occupy the same physical square. This matters most in
   the three-Sloppy secret night, but also fixes the normal Eugene/Sloppy case
   where two sprites could silently stack on one calibrated position. Gordon is
   a separate state machine and is intentionally not part of this occupancy
   rule. If every legal neighbour is occupied, the mover waits in place. */
function unitNodeOccupied(node, except){
  return !!(S.units || []).find(u => u !== except && u.node === node);
}
function stepTowardUnit(u, targets){
  const opts = ADJ[u.node].filter(n => walkable(n) && !unitNodeOccupied(n,u));
  if (!opts.length) return u.node;
  let best = opts[0], bestD = Infinity;
  for (const o of opts){
    const d = distFrom(o);
    const v = Math.min(...targets.map(t => d[t] === undefined ? 99 : d[t]));
...
function retreatFrom(node, steps, allowCross, unit){
  if (allowCross && DOOR_SIDE[node] && Math.random() < CONFIG.crossDoorChance){
    const sibling = ADJ[node].find(n => DOOR_SIDE[n] && n !== node && !unitNodeOccupied(n,unit));
    if (sibling){
      node = sibling;
  ...
  for (let i = 0; i < steps; i++){
    const free = n => walkable(n) && !unitNodeOccupied(n,unit);
    const away = ADJ[node].filter(n => free(n) && DO[n] > DO[node]);
    const side = ADJ[node].filter(n => free(n) && DO[n] >= DO[node]);
    const opts = away.length ? away : (side.length ? side : ADJ[node].filter(free));
...
function unitSpriteReady(u){
  if (u && u.specialSloppy) return true; // custom asset has its own procedural placeholder
  return !!(u && sprites[u.id] && sprites[u.id].dataset.ok === "1");
}
...
function playWalkBy(u, fromNode){
  ...
  if (!box) return;
  const img = box.querySelector(".lurker");
  if (unitSpriteReady(u)) setUnitImage(img, u);
  applyDoorShot(img, u, side);
  ...
  /* The office pass-by can remain visible for the full 2.6 seconds without
     blinding every camera for that entire animation. The benchmark showed the
     old 2.6-second global blackout, not Eugene's 420 ms move flicker, was the
     dominant source of impossible Gordon/Sloppy overlaps for a human-visible
     bot. */
  S.transitUntil = performance.now() + CONFIG.walkByStaticMs;
...
function darkRoll(u){
  ...
  u.node = retreatFrom(u.node, CONFIG.passBySteps, true, u);
...
function oneStep(from, approach, unit){
  const free = n => walkable(n) && !unitNodeOccupied(n,unit);
  const closer = ADJ[from].filter(n => free(n) && DO[n] === DO[from] - 1);
  const other  = ADJ[from].filter(n => free(n) && DO[n] >= DO[from]);
...
function lureSloppyFromDoor(u, lure){
  ...
  const next = stepTowardUnit(u, lure.nodes);
  ...
  footstep(u);
  pingSensor(u, was);
  if (u.moveTell) play(u.moveTell, { vol:S.monUp ? 0.60 : 0.48 });
  if (u.commitModel && BOT.on) botHearSloppyMove(u, was);
  return true;
}
...
function moveUnit(u, targetLure = null, forceOffice = false){
  ...
  if (targetLure){
    const next = stepTowardUnit(u, targetLure.nodes);
    u.node = next;
    u.lureTarget = targetLure.nodes;
    u.lureMoves = u.commitModel ? Math.max(0, CONFIG.lureCommitMoves - 1) : Math.max(0, EUGENE.lureCommitMoves - 1);
    u.lastLure = performance.now();
    if (DO[u.node] === 1) beginBreach(u);
    return;
  }

  u.node = oneStep(u.node, approach, u);
  if (DO[u.node] > 1 && Math.random() < dbl) u.node = oneStep(u.node, approach, u);
  if (DO[u.node] === 1) beginBreach(u);
}
...
function stepUnits(dt){
  for (const u of S.units){
    ...
    /* Watching no longer slows the clock. Sloppy instead banks attention
       between movement opportunities. Switching cameras does not erase it.
       Static does NOT pause this bank: if the player has the correct camera
       selected, the five seconds still count even while Eugene/transit snow
       temporarily obscures the picture. This keeps an unrelated visual event
       from randomly invalidating otherwise-correct Sloppy play. */
    const seenNow = watchingNode(u.node);
    if (u.commitModel && seenNow && !S.doom){
      const required = sloppyCfg().watchMs;
      const before = u.watchProgress || 0;
      u.watchProgress = Math.min(required, before + dt);
      if (!u.watchReadyTell && before < required && u.watchProgress >= required){
        u.watchReadyTell = true;
        play("sloppyReady", { vol:0.82 });
        /* Visual confirmation only. Do NOT use S.flicker here: that flag means
           Eugene moved to the bot and to several other systems. */
        S.sloppyConfirmFlicker = 140;
      }
    }
    ...
    u.acc += tick;
    const scfg = u.commitModel ? sloppyCfg() : null;
    const paceMult = scfg ? scfg.moveMult : 1;
    const paceBase = scfg
      ? (scfg.directInterval ? scfg.moveBaseMs
                             : scfg.moveBaseMs * Math.pow(1 - RAMP.moveMs, Math.min(5, S.hour)))
      : tuned().moveMs;
    if (u.acc >= paceBase * paceMult * u.rhythm * u.jitter){
      u.acc = 0;
      u.jitter = 0.85 + Math.random() * 0.30;

      const pending = u.pendingLure;
      u.pendingLure = null;
      const enoughWatch = !u.commitModel || u.watchProgress >= sloppyCfg().watchMs;

      /* Sloppy is now an observation check, not a timer stall. If you have not
         earned enough attention before her opportunity, she ALWAYS advances.
         If you have earned it, she stays unless a queued cue succeeds. */
      if (u.commitModel && !enoughWatch){
        u.watchProgress = 0;
        u.watchReadyTell = false;
        /* Missing the observation check breaks any older sound commitment.
           Sloppy's rule is absolute: without the five seconds she advances on
           the office regardless of what worked on a previous cycle. */
        u.lureMoves = 0; u.lureTarget = null;
        const was = u.node;
        moveUnit(u, null, true);
        if (u.node !== was){
          if (GRAPH[was].outside && !GRAPH[u.node].outside) play(u.enterSfx);
          footstep(u); pingSensor(u, was);
          if (u.moveTell) play(u.moveTell, { vol:S.monUp ? 0.60 : 0.48 });
          if (u.commitModel && BOT.on) botHearSloppyMove(u, was);
        }
      } else {
        let chosenLure = null;
        let continuingLure = false;
        if (pending){
          const odds = lureOdds(u, pending);
          if (Math.random() < odds) chosenLure = pending;
        }
        /* A successful cue is still worth several movements, as the tuning
           comments promise. The queued-audio redesign accidentally set this
           counter but never consumed it, which both prevented future cues and
           failed to move the character toward the old target. Continue that
           commitment here, one movement opportunity at a time. */
        if (!chosenLure && u.lureMoves > 0 && u.lureTarget){
          chosenLure = { nodes:u.lureTarget };
          continuingLure = true;
        }
        u.watchProgress = 0;
        u.watchReadyTell = false;

        if (u.commitModel && !chosenLure){
          /* Enough watch time was banked. The reward is that she does not move.
             The loud growl marks the movement opportunity being spent: from this
             exact sound onward, the next five seconds of observation belong to
             the NEXT opportunity. */
          play("sloppyBlocked", { vol:0.92 });
          S.sloppyResetPulse = 320;
          if (BOT.on) botHearSloppyBlocked(u);
          continue;
        }

        const idleHere = chosenLure ? 0 : tuned().idleChance;
        if (!chosenLure && Math.random() < idleHere) continue;

        const was = u.node;
        if (continuingLure){
          const next = stepTowardUnit(u, chosenLure.nodes);
          if (next === u.node){
            u.lureMoves = 0; u.lureTarget = null;
          } else {
            u.node = next;
            u.lureMoves = Math.max(0, u.lureMoves - 1);
            if (u.lureMoves <= 0) u.lureTarget = null;
            if (DO[u.node] === 1) beginBreach(u);
          }
        } else {
          moveUnit(u, chosenLure);
        }
      if (u.node !== was){
...
function rollGordonTellLead(){
  return GORDON.tellMs + (Math.random() * 2 - 1) * GORDON.tellJitterMs;
}

function resetGordonMoveClock(g){
  g.acc = 0;
  g.telled = false;
  g.tellLeadMs = rollGordonTellLead();
}

function spawnGordon(){
  if (secretNightMode()) return null;
  if (S.night < GORDON.startNight && S.night !== 8) return null;
  const cfg = gordonCfg();
  if (!cfg || !cfg.moveMs) return null;               // disabled/not active
  return {
    ...GORDON,
    art: gordonArtForCurrentDifficulty(),
    node: pick(GORDON.startNodes),
    phase: "wait",
    payrollRedirect: false,
    redirectTarget: null,
    acc: 0,
    telled: false,
    tellLeadMs: rollGordonTellLead(),
...
function gordonRetarget(g){
  /* After a door rebuff, the first thing Gordon does is always move back toward
     the Payroll area. Only after that forced move does he choose a new access
     point. This makes door use predictable in direction without making the next
     duct predictable. */
  if (g.payrollRedirect){
    g.duct = null;
    g.redirectTarget = "b18";
    return null;
  }
...
function gordonHitsDoor(g, sealed){
  ...
  if (dl.oneShot){
    /* A one-shot edge can override the general break time. Payroll is the
       intentionally weak sacrificial door: it buys only a couple of seconds,
       then it is gone for the rest of the shift. */
    const ms = dl.breakMs !== undefined ? dl.breakMs : ((gordonCfg() && gordonCfg().breakMs) || 0);
    ...
    resetGordonMoveClock(g);
    /* Payroll still has no big break-apart sting: it simply gives way. The
       crossed-out map pin is the persistent visual proof that it is spent. */
    if (id !== "D5") play("doorBreak", { vol: 1.0 });
    if (BOT.on) botHearGordonDoorImpact(id, g, true);
    trackGameEvent("door_broken", { night_number:S.night, door:id });
    return true;
  }

  play("doorBlocked", { vol: sealed ? 1.0 : 0.9 });
  if (BOT.on) botHearGordonDoorImpact(id, g, false);
  g.penalty = GORDON.blockPenalty;
  g.payrollRedirect = true;
  g.redirectTarget = "b18";
  g.duct = null;
  rollDoorJam(sealed);
  return false;
...
function gordonStepDown(g){
  /* A door rebuff forces exactly one move toward Payroll. Once that move is made,
     Gordon immediately chooses a new access point at random from the reachable
     ducts instead of continuing the route he was previously committed to. */
  if (g.payrollRedirect){
    const target = g.redirectTarget || "b18";
    if (g.node === target){
      g.payrollRedirect = false;
      g.redirectTarget = null;
      g.duct = chooseDuctAny(g.node, S.doorShut);
      g.sealed = !g.duct;
      return;
    }
    const forced = stepTowardBlocked(g.node, target, S.doorShut);
    if (forced !== g.node){
      g.node = forced;
      g.payrollRedirect = false;
      g.redirectTarget = null;
  ...
  if (next === g.node){
    ...
    /* The next move is deliberately handled by the Payroll redirect branch
       above. Do not immediately step away and pick another duct here. */
    return;
  }
...
function stepGordon(dt){
  ...
  switch (g.phase){
    ...
    case "wait":
      g.wait -= dt;
      if (g.wait <= 0){ g.phase = "down"; resetGordonMoveClock(g); gordonRetarget(g); }
      return;
    ...
    case "inDuct":
      ...
      if (g.phaseAcc >= GORDON.inDuctMs){
        ...
        if (BOT.on) botHearGordonDuctOut(g);
    ...
    case "climbOut":
      ...
      if (g.phaseAcc >= GORDON.climbOutMs){
        g.phase = "up"; g.phaseAcc = 0; resetGordonMoveClock(g);
        gordonPingSensor(g);
      }
      return;
    ...
    case "up": {
      ...
      const requestedLead = g.tellLeadMs || GORDON.tellMs;
      /* Keep at least 450ms between the tell and the step even if Custom-20 has
         compressed an upstairs interval almost down to the four-second lead. */
      const tellLead = Math.max(450, Math.min(requestedLead, interval - 450));
      if (!g.telled && g.acc >= interval - tellLead){
        g.telled = true;
        gordonTell(g);
      }
...
function cameraStabilityEvent(amount, source){
  if (!S.running || S.outage || !S.sys.cam || !S.sys.cam.ok) return false;
  S.cameraStability = Math.max(0, (S.cameraStability ?? 100) - amount);
  if (S.cameraStability > 0) return false;
  S.cameraStability = 0;
  S.sys.cam.ok = false;
  S.wear.cam = 0;
  playSystemFault("cam");
  if (S.monUp) toggleMonitor();
  trackGameEvent("camera_crash", { night_number:S.night, source:source || "stability" });
  return true;
}

/* A normal camera-use tick is deliberately large enough that a player who
   spends most of the night on the monitor will eventually empty the meter,
   while short checks are still cheap. Event hits are extra wear on top of that. */
const rollCamFlicker = src =>
  cameraStabilityEvent(src === "feed" ? 2.25 : 7.5, src || "event");
...
function stepDarkVent(dt){
  ...
  /* THE FIRST SEVEN SECONDS ARE ACTUALLY SAFE.
     No fault roll happens and, just as importantly, no hidden wear is accrued.
     Briefly cutting the lights to answer Eugene is a free defensive action;
     ventilation pressure only begins once the player chooses to stay dark past
     the grace window. */
  if (S.darkAcc <= V.graceMs) return;

  /* Past the grace, one real second is one step of the ventilation wear ramp. */
  rollFault("vent", V, dt / 1000);
}

function stepCameraStability(dt){
  if (!S.running || S.outage) return;
  const seconds = dt / 1000;
  if (!S.sys.cam || !S.sys.cam.ok) return;

  /* CAMERA BATTERY TIMING.

     With no event hits at all, a full camera battery lasts 20 seconds of
     continuous viewing: 100 / 20 = 5 stability points per second.

     Turning the office lights off doubles that baseline drain. The monitor is
     still usable on the residual camera line, but a full battery then lasts only
     10 seconds before event hits.

     With the monitor fully DOWN, an almost-empty battery needs about three
     seconds to refill: 100 / 3 = 33.333... points per second. Switching feeds
     neither resets nor penalises this value; it is one shared camera supply.

     Eugene flickers and unstable-feed dropouts still take their event chunks on
     top of this baseline drain, so abusing a noisy feed can empty it sooner. */
  if (S.monUp && !S.panelTab){
    const baseDrainPerSec = 100 / 20;
    const lightsOffMult = S.power ? 1 : 2;
    cameraStabilityEvent(baseDrainPerSec * lightsOffMult * seconds,
                         S.power ? "usage" : "usage_dark");
  } else if (!S.monUp){
    const rechargePerSec = 100 / 3;
    S.cameraStability = Math.min(100, (S.cameraStability ?? 0) + rechargePerSec * seconds);
  }
}
  ...
  for (const id in S.sys){
    ...
    if (s.reboot > 0){
      ...
      if (s.reboot <= 0){
        ...
        if (id === "cam") S.cameraStability = 100;
...
function hallucinationWhoForFloor(floor){
  const themedGordon = Object.assign({}, GORDON, { art:gordonArtForCurrentDifficulty() });
  const choices = floor === 2
    ? [themedGordon, JEFFREY]
    : [...ROSTER, ...(all20Mode() ? [themedGordon, JEFFREY] : [])];
...
function createHallucination(node, who, camId){
  ...
  s.style.zIndex = subjectZ(node, camId);
  setUnitImage(s, who); s.alt = "";
  s.style.left = shot.left + "%";
  s.style.bottom = shot.bottom + "%";
  s.style.width = shot.width + "%";
  s.style.height = "auto";
  s.style.transform = shotTransform(shot);
  s.style.filter = secretNightMode()
    ? "grayscale(.66) sepia(.12) hue-rotate(154deg) saturate(.78) contrast(1.18) brightness(" + (0.56 * shot.dim) + ")"
    : "grayscale(.6) contrast(1.1) brightness(" + (0.65 * shot.dim) + ")";
...
function loop(now){
  ...
  if (S.running){
    ...
    stepPower(dt);
    if (S.running && !S.outage) stepCameraStability(dt);
...
function render(dt){
  ...
  if (el.camStability){
    const stab = Math.max(0, Math.min(100, S.cameraStability ?? 0));
    /* 81-100 = five, 61-80 = four, 41-60 = three, 21-40 = two,
       anything above zero through 20 = one. At zero the array is already down. */
    const bars = stab <= 0 ? 0 : Math.min(5, Math.floor((stab - 0.000001) / 20) + 1);
    el.camStability.querySelectorAll(".camBattery i").forEach((b, i) => {
      b.classList.toggle("on", i < bars);
    });
    el.camStability.classList.remove("state5","state4","state3","state2","state1","state0");
    el.camStability.classList.add("state" + bars);
  }
  ...
  if (S.sloppyConfirmFlicker > 0){
    S.sloppyConfirmFlicker -= dt;
    if (S.sloppyConfirmFlicker <= 0) S.sloppyConfirmFlicker = 0;
  }
  if (S.sloppyResetPulse > 0){
    S.sloppyResetPulse -= dt;
    if (S.sloppyResetPulse <= 0) S.sloppyResetPulse = 0;
  }
  document.body.classList.toggle("sloppyconfirm",
    S.monUp && S.sloppyConfirmFlicker > 0);
...
let subjKey = "";
/* Painter's order is PER CAMERA, because the same physical node can be seen
   from a completely different angle on another feed. Every list is FAR -> NEAR:
   lower indexes paint first, higher indexes paint over them.

   IMPORTANT: hallucinations use this exact same table. This prevents a ghost in
   a distant calibrated position from being appended last and incorrectly
   painting over a real animatronic that is physically closer to the lens.

   The multi-position upper-floor runs preserve the hand-authored orders that
   existed before this table. Cameras that never had an explicit painter order
   start in their camera `sees` declaration order (with `peek` positions first,
   because those are intentionally distant/edge-of-frame). They are all exposed
   in the calibrator so the photographic depth can be corrected without code. */
const CAMERA_DRAW_ORDER = {
  C01:["n01"],
  C02:["n02"],
  C03:["n03"],
...
function depthOf(node, camId){
  const order = CAMERA_DRAW_ORDER[camId || S.activeCam] || [];
  const i = order.indexOf(node);
  return i < 0 ? -1 : i;
}
function subjectZ(node, camId){
  return 10 + depthOf(node, camId);
}
...
function drawSubjects(force){
  ...
  solid.slice().sort((a, b) => depthOf(a.node, S.activeCam) - depthOf(b.node, S.activeCam))
       .forEach(u => addSubject(u, shotFor(u, u.node, S.activeCam), 1, u.node));
  ...
  if (gSolid || gFaint){
    ...
    addSubject(S.gordon, shot, gFaint ? 0.34 : 1, node);
  }
}

function addSubject(u, shotIn, alpha, depthNode){
  if (u.ghost) return;
  if (!unitSpriteReady(u)) return;
  ...
  s.style.zIndex = subjectZ(depthNode || u.node, S.activeCam);
  setUnitImage(s, u); s.alt = "";
  s.style.left   = shot.left + "%";
  s.style.bottom = shot.bottom + "%";
  s.style.width  = shot.width + "%";
  s.style.height = "auto";
...
/* ===========================================================================
   12c. AUTOPILOT — PERFECT-PLAYER BENCHMARK

   Press B to hand the office over. This is a balance benchmark rather than a
   casual demo: it minimises main power, camera battery and wasted attention,
   while reacting to the same audible tells a player receives.

   It is not clairvoyant about future random rolls. Gordon's tell starts a
   camera search; his position only enters memory after the feed that actually
   shows him is opened. Sloppy's entry/movement sounds work the same way. The
   "perfect" part is exact memory and execution — it never forgets the last
   camera/node it actually saw and searches that neighbourhood first.
   =========================================================================== */

/* false = benchmark only information actually visible through the rendered feed.
   true is retained as a diagnostic comparison mode, not the release behavior. */
const BOT_CHEATS = false;
const BOT = {
  on:false, acc:0, dwell:0, mem:{},
  sloppyEntered:false, sloppyNeeds:false, sloppySearch:[], sloppySearchStarted:false, sloppyInitialCueDone:false, sloppyRecharge:false,
  sloppyLatestStart:0, sloppyCycleAt:0, sloppyCyclesSinceCue:0,
  cueAim:null,
  gordonAlert:false, gordonSearch:[], gordonSearchStarted:false, gordonHeldDoor:null, gordonReleaseDoor:false,
  gordonUp:false, gordonStepsSinceSeen:0, gordonDeadline:0, gordonForcedNext:null,
  idlePowerOnUntil:0
};

const BOT_DARK_MAX_MS = 6300;       // use most of the official 7.0s safe-dark grace, leaving reaction margin
const BOT_LIGHT_RESET_MS = 180;     // shortest clear visual beat; then go dark again
const BOT_CAM_MIN_SLOPPY = 80;      // 5s on residual cameras costs ~50%; keep event margin
const BOT_CAM_SEARCH_DWELL = 80;    // perfect-player snap check; still one feed at a time

function toggleBot(){
  if (!BOT.on && secretNightMode()){ buzz(); return; }
  BOT.on = !BOT.on;
  BOT.acc = BOT.dwell = 0; BOT.mem = {};
  BOT.sloppyEntered = false; BOT.sloppyNeeds = false; BOT.sloppySearch = []; BOT.sloppySearchStarted = false;
  BOT.sloppyInitialCueDone = false; BOT.sloppyRecharge = false; BOT.sloppyLatestStart = 0; BOT.sloppyCycleAt = 0; BOT.sloppyCyclesSinceCue = 0; BOT.cueAim = null;
  BOT.gordonAlert = false; BOT.gordonSearch = []; BOT.gordonSearchStarted = false; BOT.gordonHeldDoor = null;
  BOT.gordonReleaseDoor = false; BOT.gordonUp = false; BOT.gordonStepsSinceSeen = 0; BOT.gordonDeadline = 0; BOT.gordonForcedNext = null; BOT.idlePowerOnUntil = 0;
  if (BOT.on){
    const s = S.units.find(u => u.commitModel);
    if (s && !GRAPH[s.node].outside){ BOT.sloppyEntered = true; BOT.sloppyNeeds = true; botScheduleSloppyCycle(s); }
    if (S.gordon && floorOf(S.gordon.node) === 1) BOT.gordonUp = true;
  }
  document.body.classList.toggle("bot", BOT.on);
  blip();
}

/* The bot never assumes it got a friendly jitter roll. This is the earliest
   wall-clock interval Sloppy could legally receive at the HARDEST hourly ramp,
   including her 0.85 jitter floor and speed modifier. Using the 5 AM ramp even
   at midnight is intentionally conservative: if the benchmark can win on this
   schedule, a human never loses because the bot secretly knew the random timer. */
function botSloppyWorstCaseIntervalMs(u){
  if(!u) return 999999;
  const c=sloppyCfg();
  const base=c.moveBaseMs * Math.pow(1 - RAMP.moveMs, 5);
  return base * c.moveMult * u.rhythm * 0.85 / Math.max(0.001,u.speedMod);
}
function botScheduleSloppyCycle(u){
  const now=performance.now();
  BOT.sloppyCycleAt=now;
  /* 350ms covers the snap back to the remembered camera. Starting this early
     is the benchmark's guarantee that five seconds can always be banked. */
  BOT.sloppyLatestStart=now + Math.max(0, botSloppyWorstCaseIntervalMs(u) - sloppyCfg().watchMs - 350);
}

/* ---- The events a perfect player can hear -------------------------------- */
function botHearSloppyMove(u, was){
  if (!BOT.on || !u || !u.commitModel) return;
  const entered = GRAPH[was] && GRAPH[was].outside && GRAPH[u.node] && !GRAPH[u.node].outside;
  if (entered) BOT.sloppyEntered = true;
  if (!BOT.sloppyEntered) return;
  BOT.sloppyNeeds = true; BOT.sloppySearch = []; BOT.sloppySearchStarted = false; BOT.cueAim = null;
  BOT.sloppyRecharge = false;
  BOT.sloppyCyclesSinceCue = (BOT.sloppyCyclesSinceCue || 0) + 1;
  botScheduleSloppyCycle(u);
}
function botHearSloppyBlocked(u){
  if (!BOT.on || !u || !u.commitModel) return;
  BOT.sloppyEntered = true; BOT.sloppyNeeds = true; BOT.sloppySearch = []; BOT.sloppySearchStarted = false; BOT.cueAim = null;
  BOT.sloppyRecharge = true;
  BOT.sloppyCyclesSinceCue = (BOT.sloppyCyclesSinceCue || 0) + 1;
  botScheduleSloppyCycle(u);
}
function botGordonPossibleNodesFromMemory(){
  const m=BOT.mem.gordon;
  if(!m || !m.node || floorOf(m.node)!==2) return null;
  let possible=new Set([m.node]);
  const steps=Math.min(6, BOT.gordonStepsSinceSeen||0);
  for(let i=0;i<steps;i++){
    const next=new Set();
    possible.forEach(n => (ADJ[n]||[]).forEach(x => {
      if(floorOf(x)===2 && walkable(x)) next.add(x);
    }));
    if(next.size) possible=next;
  }
  return possible;
}
function botGordonDoorRiskFromMemory(){
  const possible=botGordonPossibleNodesFromMemory();
  if(!possible) return true; // unknown = must look
  return [...possible].some(n => DOOR_LINKS.some(d =>
    !S.doorBroken[d.id] && (d.a===n || d.b===n)));
}
/* Minimum ordinary steps from the last legitimately known Gordon position(s)
   to ANY still-live door endpoint. Zero means he could cross a door on his
   next movement. Infinity means the remembered region is safely separated from
   every remaining door for now. */
function botGordonStepsToNearestDoor(){
  const possible=botGordonPossibleNodesFromMemory();
  if(!possible) return 0;
  const endpoints=[];
  DOOR_LINKS.forEach(d=>{ if(!S.doorBroken[d.id]){ endpoints.push(d.a,d.b); } });
  if(!endpoints.length) return Infinity;
  let best=Infinity;
  possible.forEach(n=>{
    const d=distFrom(n);
    endpoints.forEach(e=>{ if(d[e]!==undefined) best=Math.min(best,d[e]); });
  });
  return best;
}

function botHearGordonTell(g){
  if (!BOT.on || !g) return;
  if(g.phase !== "down"){ BOT.gordonAlert=false; return; }

  /* After a successful ordinary-door block the game rule is deterministic:
     Gordon's NEXT step is toward Payroll before he chooses another access
     point. A perfect player can predict that one step from the door they just
     heard him hit; no camera search is required unless that forced edge itself
     crosses another live door. */
  if(BOT.gordonForcedNext && BOT.mem.gordon && BOT.mem.gordon.node){
    const from=BOT.mem.gordon.node, to=BOT.gordonForcedNext;
    const d=DOOR_LINKS.find(x=>!S.doorBroken[x.id]&&((x.a===from&&x.b===to)||(x.b===from&&x.a===to)));
    if(!d){ BOT.gordonAlert=false; BOT.gordonDeadline=0; BOT.gordonSearch=[]; return; }
  }

  /* During an oxygen-critical ventilation reboot, do not throw away six
     seconds of completed work just because Gordon shifted his weight somewhere
     we know cannot reach a live door on this step. This uses only the last
     camera position actually seen plus the number of audible footsteps since. */
  const ventCritical=S.sys.vent && S.sys.vent.reboot>0 && S.o2<=85;
  if(ventCritical && !botGordonDoorRiskFromMemory()){
    BOT.gordonAlert=false;
    return;
  }
  /* Perfect-player optimisation: if Gordon's last actually-observed position,
     plus every audible step since then, proves he cannot possibly cross a live
     door on THIS move, do not burn camera battery or steal five seconds from
     Sloppy just to confirm a harmless hallway step. Unknown/ambiguous memory
     still forces a camera check. */
  if(!botGordonDoorRiskFromMemory()){
    BOT.gordonAlert=false; BOT.gordonDeadline=0; BOT.gordonSearch=[]; BOT.gordonSearchStarted=false;
    return;
  }

  BOT.gordonAlert=true;
  const cfg=gordonCfg();
  const mult=(g.phase === "up" ? GORDON.upMult : 1) * (1 + g.penalty);
  const interval=cfg.moveMs * mult;
  const req=g.tellLeadMs || GORDON.tellMs;
  const lead=Math.max(450, Math.min(req, interval - 450));
  BOT.gordonDeadline=performance.now()+lead;
  BOT.gordonSearch=[]; BOT.gordonSearchStarted=false;
}
function botHearGordonStep(g){
  if (!BOT.on || !g) return;
  BOT.gordonDeadline=0;
  if (floorOf(g.node) === 1){ BOT.gordonUp = true; BOT.gordonAlert = false; BOT.gordonForcedNext=null; }
  else if(BOT.gordonForcedNext){
    /* This is not clairvoyance: the forced Payroll step is a public rule and
       the bot knew the exact door/node it had just blocked. Keep memory exact
       without spending camera battery to rediscover a deterministic move. */
    const m=BOT.mem.gordon||{};
    BOT.mem.gordon={cam:m.cam||null,node:BOT.gordonForcedNext,t:performance.now(),via:"predicted"};
    BOT.gordonForcedNext=null;
    BOT.gordonStepsSinceSeen=0;
  } else BOT.gordonStepsSinceSeen++;
  if (BOT.gordonHeldDoor) BOT.gordonReleaseDoor = true; // stray step: armed edge wasn't used
}
function botHearGordonDoorImpact(id, g, broken){
  if (!BOT.on) return;
  BOT.gordonAlert = false; BOT.gordonDeadline = 0; BOT.gordonSearch = []; BOT.gordonSearchStarted = false;
  BOT.gordonForcedNext = null;
  if(!broken && g && floorOf(g.node)===2){
    const old=BOT.mem.gordon||{};
    BOT.mem.gordon={cam:old.cam||null,node:g.node,t:performance.now(),via:"door"};
    BOT.gordonStepsSinceSeen=0;
    BOT.gordonForcedNext=stepTowardBlocked(g.node,"b18",null);
    if(BOT.gordonForcedNext===g.node) BOT.gordonForcedNext=null;
  }
  if (BOT.gordonHeldDoor === id){
    if (broken || S.doorShut !== id) BOT.gordonHeldDoor = null;
    else BOT.gordonReleaseDoor = true;
  }
}
function botHearGordonDuctOut(g){
  if (!BOT.on) return;
  BOT.gordonUp = true; BOT.gordonAlert = false; BOT.gordonDeadline = 0; BOT.gordonSearch = []; BOT.gordonSearchStarted=false; BOT.gordonForcedNext=null;
}

/* ---- Memory and search ---------------------------------------------------- */
function botRemember(id, cam, node, via){
  BOT.mem[id] = {cam,node,t:performance.now(),via:via||"seen"};
  if(id === "gordon") BOT.gordonStepsSinceSeen = 0;
}
function botGordonNodeOnCurrentFeed(){
  const n=gordonVisibleNode();
  return n && CAMERAS[S.activeCam] && CAMERAS[S.activeCam].sees.includes(n) ? n : null;
}
function botFeedReadable(){
  /* BOT_CHEATS exists specifically so the benchmark can compare an omniscient
     diagnostic bot against a human-legible one. In the normal benchmark it is
     false: Eugene snow, doorway transit snow and unstable-feed dropouts hide
     exactly the same information from the bot that they hide from the player. */
  return BOT_CHEATS || !feedCoveredByStatic();
}
function botRememberCurrentFeed(){
  if (!S.monUp || !S.sys.cam.ok || !botFeedReadable()) return false;
  unitsSeenBy(S.activeCam).forEach(u => botRemember(u.id,S.activeCam,u.node,"seen"));
  const n=botGordonNodeOnCurrentFeed(); if (n) botRemember("gordon",S.activeCam,n,"seen");
  return true;
}
function botSearchList(id,floor){
  const out=[], add=c=>{ if(c&&CAMERAS[c]&&camOnFloor(c,floor)&&!camDead(c)&&!out.includes(c)) out.push(c); };
  const m=BOT.mem[id];
  if (m){
    add(m.cam);
    if (m.node && ADJ[m.node]) [m.node,...ADJ[m.node]].forEach(n =>
      Object.keys(CAMERAS).forEach(c=>{ if((CAMERAS[c].sees||[]).includes(n)) add(c); }));
  } else if(id === "sloppy" && floor === 1){
    /* Her three entrances collapse to two feeds: C04 sees north-west + east,
       C14 sees the south entrance. Entry audio therefore needs at most two
       snap checks instead of a sweep of the entire upper floor. */
    add("C04"); add("C14");
  }
  camIdsOn(floor).filter(c=>!CAMERAS[c].unstable).forEach(add);
  camIdsOn(floor).filter(c=> CAMERAS[c].unstable).forEach(add);
  return out;
}
function botTakeSearchStep(id,floor){
  const key=id==="gordon"?"gordonSearch":"sloppySearch";
  const startedKey=id==="gordon"?"gordonSearchStarted":"sloppySearchStarted";

  /* One audible movement/tell earns ONE finite sweep. Some graph squares have
     intentionally no camera coverage. Rebuilding the list after the final feed
     made the autopilot loop forever when Gordon/Sloppy happened to be standing
     in one of those blind rooms. A human would finish the sweep, accept that the
     target is currently unviewable, and wait for the next movement sound. */
  if (!BOT[startedKey]){
    BOT[key]=botSearchList(id,floor);
    BOT[startedKey]=true;
  }
  if (!BOT[key].length) return false;
  const c=BOT[key].shift();
  if(S.activeCam!==c) viewCam(c);
  BOT.dwell=BOT_CAM_SEARCH_DWELL;
  return true;
}

/* ---- Lure choices --------------------------------------------------------- */
function lureCamAway(u){
  const d=distFrom(u.node); let best=null,bestScore=Infinity;
  for(const id in CAMERAS){
    const c=CAMERAS[id]; if(c.unstable||camDead(id)) continue;
    const out=Math.max(...c.sees.map(x=>DO[x]===undefined?0:DO[x])); if(out<5) continue;
    const rooms=Math.min(...c.sees.map(x=>d[x]===undefined?99:d[x])); if(rooms<1||rooms>5) continue;
    const score=rooms*10-out; if(score<bestScore){bestScore=score;best=id;}
  }
  return best;
}
function cueCamFor(u){
  const d=distFrom(u.node); let best=null,bestRooms=99;
  for(const id in CAMERAS){
    const c=CAMERAS[id]; if(c.unstable||c.sees.includes(u.node)||camDead(id)) continue;
    const rooms=Math.min(...c.sees.map(x=>d[x]===undefined?99:d[x])); if(rooms<1||rooms>3) continue;
    if(rooms<bestRooms){bestRooms=rooms;best=id;}
  }
  return best;
}

/* ---- Gordon: tell -> locate -> arm exact edge ----------------------------- */
function botPlannedGordonNext(g){
  if(!g||g.phase!=="down") return null;
  if(g.payrollRedirect){ const t=g.redirectTarget||"b18"; return g.node===t?null:stepTowardBlocked(g.node,t,null); }
  const t=g.duct; if(!t||g.node===t.down) return null;
  return stepTowardBlocked(g.node,t.down,null);
}
function botDoorForGordon(g){
  const n=botPlannedGordonNext(g); if(!n||n===g.node) return null;
  return DOOR_LINKS.find(d=>!S.doorBroken[d.id]&&((d.a===g.node&&d.b===n)||(d.b===g.node&&d.a===n)))||null;
}

function botEugeneAtDoor(){
  return S.units.find(u => u.breaching && !u.commitModel) || null;
}

/* Eugene at the doorway is an absolute interrupt. A perfect player does not
   finish a vent reboot, stare at a panel or preserve a housekeeping task while
   the one defense Eugene cares about is wrong. Killing the mains also aborts
   any reboot, exactly as it does for a human pressing X. */
function botForceDarkForEugene(){
  if(!botEugeneAtDoor()) return false;
  if(rebootActive()) cancelReboots();
  if(S.panelOpen && S.power) togglePanel();

  /* Eugene only cares about the lights. If the monitor is already on the feed
     containing Sloppy, keep watching on residual camera power instead of
     throwing away a nearly-complete five-second bank. Otherwise drop it. */
  const knownSloppy=S.units.find(u=>u.commitModel);
  const sloppyHere = S.monUp && BOT.sloppyNeeds && S.sys.cam.ok
    ? (botFeedReadable()
       ? unitsSeenBy(S.activeCam).find(u=>u.commitModel)
       : (knownSloppy && BOT.mem.sloppy && BOT.mem.sloppy.cam===S.activeCam ? knownSloppy : null))
    : null;
  const remain = sloppyHere ? Math.max(0, sloppyCfg().watchMs - (sloppyHere.watchProgress||0)) : 0;
  /* Residual cameras drain at 10 stability points/sec. Preserve an in-progress
     Sloppy watch only when the battery can actually finish it with a small
     margin; otherwise drop the monitor and use Eugene's visit as a recharge
     window instead of crashing the camera system for a doomed partial watch. */
  const preservingSloppy = !!sloppyHere && (S.cameraStability??0) >= (remain/1000)*10 + 35;
  if(S.monUp && !preservingSloppy && !rebootActive()) toggleMonitor();
  if(S.power) togglePower();
  return true;
}

function botDropToDark(){
  if(BOT.gordonHeldDoor) return false;
  if(S.monUp && !rebootActive()) toggleMonitor();
  if(S.panelOpen && S.power && !rebootActive()) togglePanel();
  if(S.power) togglePower();
  return true;
}

function botNeedPower(){
  if(S.power) return true;
  if(performance.now()>=S.powerBackAt) togglePower();
  return false;
}
function botActiveRebootIds(){ return SYSTEMS.filter(s=>S.sys[s.id].reboot>0).map(s=>s.id); }
function botHandleReboot(id){
  if(!botNeedPower()) return true;
  if(S.monUp){toggleMonitor();return true;}
  if(!S.panelOpen){togglePanel();return true;}
  const x=S.sys[id]; if(x&&x.reboot<=0) reboot([id]); return true;
}

function botHandleDoorRelease(){
  if(!BOT.gordonHeldDoor&&!BOT.gordonReleaseDoor) return false;
  const id=BOT.gordonHeldDoor;
  if(!id||S.doorShut!==id){
    BOT.gordonHeldDoor=null; BOT.gordonReleaseDoor=false;
    /* If the impact jammed the array, do NOT immediately kill the mains and
       incur the breaker warm-up. Keep power available so the six-second repair
       can begin on the next bot tick. Eugene can still force darkness if he
       arrives, but the bot no longer wastes free repair time by choice. */
    const doorNeedsRepair=!BOT.gordonUp && S.gordon && S.gordon.phase === "down" &&
                          S.sys.door && !S.sys.door.ok;
    if(S.power && !botEugeneAtDoor() && !doorNeedsRepair) togglePower();
    return false;
  }
  if(!BOT.gordonReleaseDoor) return false;
  if(performance.now()<S.doorReadyAt) return true;

  /* Releasing is fail-open, so it works even while Eugene is forcing the
     office dark. Do it before any other housekeeping; every extra frame here
     is needless door power. */
  toggleLastDoor();
  BOT.gordonHeldDoor=null; BOT.gordonReleaseDoor=false;
  const doorNeedsRepair=!BOT.gordonUp && S.gordon && S.gordon.phase === "down" &&
                        S.sys.door && !S.sys.door.ok;
  if(S.doorShut===null && S.power && !botEugeneAtDoor() && !doorNeedsRepair) togglePower();
  return true;
}

function botCanFinishSloppyBeforeGordon(){
  if(!BOT.gordonAlert || !BOT.sloppyNeeds || !S.monUp || BOT.gordonDeadline<=0) return false;
  const s=S.units.find(u=>u.commitModel);
  if(!s || s.breaching || !unitsSeenBy(S.activeCam).includes(s)) return false;
  const remaining=Math.max(0, sloppyCfg().watchMs-(s.watchProgress||0));
  const now=performance.now();
  const timeLeft=BOT.gordonDeadline-now;
  const drainPerSec=S.power ? 5 : 10;
  const stabilityAfter=(S.cameraStability??0) - (remaining/1000)*drainPerSec;
  /* Finishing Sloppy is only safe if Gordon still has his full mechanical
     response budget afterwards. In particular, a dark office can be inside
     the breaker warm-up: those 2.6 seconds have to be counted BEFORE the door
     motor can move, not wished away as camera-search time. */
  const powerWarm = S.power ? 0 : Math.max(0, S.powerBackAt-now);
  const gordonReserve = powerWarm + 900; // floor switch/search snap + door command
  return remaining + gordonReserve <= timeLeft && stabilityAfter >= 35;
}

function botHandleGordon(){
  if(!BOT.gordonAlert||!S.gordon||BOT.gordonUp||S.gordon.phase!=="down") return false;
  if(!S.sys.cam.ok) return botHandleReboot("cam");
  if(S.panelOpen){togglePanel();return true;}
  /* A perfect player spends a little main power here to protect the much more
     constrained camera battery. Gordon searches are brief, and the mains will
     be required anyway if the next edge is a door. Eugene still overrides this
     at the very top of stepBot. */
  if(!S.power && performance.now()>=S.powerBackAt){ togglePower(); return true; }
  if(!S.monUp){toggleMonitor();return true;}
  if(S.mapFloor!==2){setMapFloor(2);BOT.dwell=80;return true;}
  if(!botFeedReadable()){
    /* Do not sweep onward through snow. A human has to hold this feed until the
       image returns before deciding Gordon is absent. Re-arm a tiny dwell every
       tick so the next search camera is not selected underneath the static. */
    BOT.dwell=Math.max(BOT.dwell,120);
    return true;
  }
  const node=botGordonNodeOnCurrentFeed();
  if(node){
    botRemember("gordon",S.activeCam,node,"seen"); BOT.gordonSearch=[]; BOT.gordonSearchStarted=false;
    const door=botDoorForGordon(S.gordon);
    if(door){
      if(!S.sys.door.ok){BOT.gordonAlert=false;if(S.monUp)toggleMonitor();return botHandleReboot("door");}
      if(!S.power){ if(performance.now()>=S.powerBackAt) togglePower(); return true; }
      if(S.doorShut!==door.id){ if(performance.now()<S.doorReadyAt) return true; toggleDoor(door.id); }
      if(S.doorShut===door.id){BOT.gordonHeldDoor=door.id;BOT.gordonReleaseDoor=false;}
    }
    BOT.gordonAlert=false;
    if(S.monUp) toggleMonitor();
    if(!BOT.gordonHeldDoor && S.power && !botEugeneAtDoor()) togglePower();
    return true;
  }
  if(BOT.dwell>0) return true;
  if(botTakeSearchStep("gordon",2)) return true;

  /* Full lower-floor sweep completed with no visible Gordon. He is in one of
     the camera-blind squares (or between visible positions). Preserve the last
     genuinely seen camera/node in BOT.mem and stop spending battery. His next
     tell/footstep will start a fresh sweep from that remembered neighbourhood. */
  BOT.gordonAlert=false; BOT.gordonDeadline=0; BOT.gordonSearch=[]; BOT.gordonSearchStarted=false;
  if(S.monUp) toggleMonitor();
  if(!BOT.gordonHeldDoor && S.power && !botEugeneAtDoor()) togglePower();
  return true;
...
/* ---- Sloppy: movement sound -> locate -> exactly 5s ----------------------- */
function botHandleSloppy(){
  if(!BOT.sloppyEntered||!BOT.sloppyNeeds) return false;
  const her=S.units.find(u=>u.commitModel); if(!her){BOT.sloppyNeeds=false;return false;}
  if(!S.sys.cam.ok) return botHandleReboot("cam");

  /* A successful block creates a short recharge pocket. Use it. With powered
     cameras a five-second watch costs about 25 stability; waiting until ~94
     here prevents the slow ratchet toward an unavoidable camera crash. */
  if(BOT.sloppyRecharge && !her.breaching){
    const now=performance.now();
    if(now < BOT.sloppyLatestStart){
      if(S.monUp){ toggleMonitor(); return true; }
      if(S.power && !BOT.gordonHeldDoor){ togglePower(); return true; }
      /* Recharge only inside genuine spare time. Never let a pretty 100% meter
         steal the five-second observation window. */
      if((S.cameraStability??100) < 98 && now + 120 < BOT.sloppyLatestStart) return true;
    }
    BOT.sloppyRecharge=false;
  }

  if(!her.breaching&&!S.monUp&&(S.cameraStability??100)<BOT_CAM_MIN_SLOPPY &&
     performance.now() + 120 < BOT.sloppyLatestStart){
    if(S.power && !BOT.gordonHeldDoor) togglePower();
    return true;
  }
  const eugeneDoor=S.units.some(u=>u.breaching&&!u.commitModel);
  if(S.panelOpen){togglePanel();return true;}
  /* Powered viewing halves camera-battery drain. The monitor is the scarce
     resource in All-20; spend the much cheaper main-power trickle during an
     active Sloppy job, then go fully dark the instant the job is done. */
  if(!S.power && !eugeneDoor && performance.now()>=S.powerBackAt){ togglePower(); return true; }
  if(!S.monUp){toggleMonitor();return true;}
  if(S.mapFloor!==1){setMapFloor(1);BOT.dwell=80;return true;}
  if(!botFeedReadable()){
    BOT.dwell=Math.max(BOT.dwell,120);
    return true;
  }
  botRememberCurrentFeed();
  const visible=S.sys.cam.ok?unitsSeenBy(S.activeCam):[];
  const sloppy=visible.find(u=>u.commitModel);

  if(BOT.cueAim){
    if(!S.sys.audio.ok||performance.now()<S.lureReadyAt){BOT.cueAim=null;BOT.sloppyNeeds=false;if(S.monUp)toggleMonitor();return true;}
    if(!S.power){ if(performance.now()>=S.powerBackAt)togglePower(); else if(S.monUp)toggleMonitor(); return true; }
    if(S.activeCam!==BOT.cueAim){viewCam(BOT.cueAim);BOT.dwell=80;return true;}
    playCue(); BOT.sloppyInitialCueDone=true; BOT.sloppyCyclesSinceCue=0; BOT.cueAim=null; BOT.sloppyNeeds=false;
    if(S.monUp && !BOT.gordonAlert)toggleMonitor();
    if(S.power && !BOT.gordonHeldDoor && !BOT.gordonAlert) togglePower();
    return true;
  }

  if(sloppy){
    botRemember("sloppy",S.activeCam,sloppy.node,"seen"); BOT.sloppySearch=[]; BOT.sloppySearchStarted=false;
    /* Watching Sloppy is the one camera job worth lighting the office for when
       Eugene is not present. Camera stability drains at half the residual-power
       rate, and the bot still kills the lights instantly if Eugene arrives. */
    if(!sloppy.watchReadyTell){
      if(!botEugeneAtDoor() && !S.power && performance.now()>=S.powerBackAt) togglePower();
      return true;
    }
    /* If Gordon is already in his four-second warning window, the five-second
       bank is the only thing Sloppy needed. Do not spend another camera switch
       on a lure and, crucially, do not cut the mains: Gordon may need a door
       motor before the breaker warm-up would finish. */
    if(BOT.gordonAlert){
      BOT.sloppyNeeds=false;
      return true;
    }
    /* AUDIO IS NOW A REGULAR TOOL, NOT ONLY A PANIC BUTTON.
       A perfect player with healthy power should spend some of that reserve to
       keep Sloppy away from the office. At high power the bot is willing to cue
       every completed cycle; below that it spaces cues to roughly every third
       cycle, while a close Sloppy always justifies the cost. This remains
       adaptive rather than blindly spending ~2% on every possible cooldown. */
    const sloppyDistance=DO[sloppy.node]===undefined?99:DO[sloppy.node];
    const cueReserve=all20Mode()?32:24;
    const cadence=(S.pw>=62)?1:2;
    const canUseFreshCue=(sloppy.lureMoves||0)<=0 && !sloppy.pendingLure;
    const shouldCue=canUseFreshCue && (!BOT.sloppyInitialCueDone || sloppyDistance<=5 ||
      (S.pw>cueReserve && (BOT.sloppyCyclesSinceCue||0)>=cadence));
    if(shouldCue&&!eugeneDoor&&S.sys.audio.ok&&performance.now()>=S.lureReadyAt){
      const aim=lureCamAway(sloppy);
      if(aim){BOT.cueAim=aim;if(!S.power&&performance.now()>=S.powerBackAt)togglePower();if(S.activeCam!==aim)viewCam(aim);return true;}
    }
    BOT.sloppyNeeds=false;
    if(S.monUp && !BOT.gordonAlert)toggleMonitor();
    if(S.power && !BOT.gordonHeldDoor && !BOT.gordonAlert) togglePower();
  ...
  if(BOT.dwell>0) return true;
  if(botTakeSearchStep("sloppy",1)) return true;

  /* Sloppy can also occupy an upper-floor square no camera covers. Do not loop
     through every feed forever. Keep the old remembered camera intact, close
     the monitor, and wait for the next audible movement to try again. */
  BOT.sloppyNeeds=false; BOT.sloppySearch=[]; BOT.sloppySearchStarted=false;
  if(S.monUp) toggleMonitor();
  if(S.power && !BOT.gordonHeldDoor && !BOT.gordonAlert && !botEugeneAtDoor()) togglePower();
  return true;
}

/* ---- Doorway fallbacks and Jeffrey --------------------------------------- */
function botHandleDoorwayThreats(){
  const b=S.units.filter(u=>u.breaching), e=b.find(u=>!u.commitModel), s=b.find(u=>u.commitModel);
  if(e){ botForceDarkForEugene(); return true; }
  if(s){
    if(!S.power){if(performance.now()>=S.powerBackAt)togglePower();return true;}
    if(S.sys.audio.ok&&performance.now()>=S.lureReadyAt){
      if(S.panelOpen){togglePanel();return true;} if(!S.monUp){toggleMonitor();return true;}
      const c=cueCamFor(s); if(c&&S.activeCam!==c){viewCam(c);return true;} if(c){playCue();return true;}
    }
    if(S.monUp)toggleMonitor(); if(S.power)togglePower(); return true;
  }
  return false;
}
function botHandleJeffrey(){
  if(!(S.jeffrey&&S.monUp&&S.jeffrey.at===S.activeCam)) return false;
  const ids=camIdsOn(S.mapFloor).filter(c=>c!==S.activeCam&&!camDead(c));
  const away=ids.find(c=>!CAMERAS[c].unstable)||ids[0]; if(away)viewCam(away); BOT.dwell=80; return true;
}

/* ---- Faults and low-power idle ------------------------------------------- */
function botSloppyWatchRemainingMs(){
  if(!BOT.sloppyNeeds) return 0;
  const s=S.units.find(u=>u.commitModel);
  if(!s || s.breaching) return 0;
  return Math.max(0, sloppyCfg().watchMs - (s.watchProgress||0));
}

/* Decide whether oxygen is tight enough that ventilation must interrupt a
   pending Sloppy watch. This is a budget, not a magic percentage: include the
   full reboot, the observation still owed, breaker warm-up if the mains are
   currently cut, and a small reaction margin. */
function botVentCannotWaitForSloppy(){
  if(!S.sys.vent || S.sys.vent.ok) return false;
  const msToZero=(S.o2/Math.max(0.001,CONFIG.o2Drain))*1000;
  const warm=!S.power ? Math.max(0,S.powerBackAt-performance.now()) : 0;
  const watch=botSloppyWatchRemainingMs();
  return msToZero <= warm + CONFIG.rebootOneMs + watch + 2200;
}

function botMaintenanceSlackMs(){
  if(!BOT.sloppyEntered || !BOT.sloppyNeeds || !BOT.sloppyLatestStart) return Infinity;
  return Math.max(0, BOT.sloppyLatestStart - performance.now());
}
function botCanStartSixSecondRepair(){
  return botMaintenanceSlackMs() >= CONFIG.rebootOneMs + 250;
}

function botHandleDowntimeFaults(){
  const f=id=>S.sys[id]&&!S.sys[id].ok&&S.sys[id].reboot<=0;
  const down=SYSTEMS.filter(s=>f(s.id)).map(s=>s.id);
  /* Eleven seconds beats two six-second cycles only marginally and is far too
     long once Sloppy is demanding checks every few seconds. Use REBOOT ALL
     only in genuinely quiet early-night downtime with multiple faults. */
  if(down.length>=2 && !BOT.sloppyEntered && !BOT.gordonAlert && !botEugeneAtDoor()){
    if(!botNeedPower()) return true;
    if(S.monUp){toggleMonitor();return true;}
    if(!S.panelOpen){togglePanel();return true;}
    if(!rebootActive()) reboot(SYSTEMS.map(s=>s.id),true);
    return true;
  }
  if(f("cam"))return botHandleReboot("cam");
  if(f("door")&&!BOT.gordonUp && botCanStartSixSecondRepair())return botHandleReboot("door");
  if(f("audio") && botCanStartSixSecondRepair())return botHandleReboot("audio");
  /* With no Sloppy job pending, ventilation is pure downtime work: repair it
     early so repeated Eugene interruptions still leave oxygen headroom. */
  if(f("vent")&&!BOT.sloppyNeeds)return botHandleReboot("vent");
  return false;
}
function botIdleDarkCycle(){
  if(botEugeneAtDoor()){ botForceDarkForEugene(); return; }
  if(S.monUp){toggleMonitor();return;}
  if(S.panelOpen){togglePanel();return;}
  if(!S.power){
    if((S.darkAcc||0)>=BOT_DARK_MAX_MS&&performance.now()>=S.powerBackAt){togglePower();BOT.idlePowerOnUntil=performance.now()+BOT_LIGHT_RESET_MS;}
    return;
  }
  if(BOT.gordonHeldDoor)return;
  if(performance.now()<BOT.idlePowerOnUntil)return;
  togglePower();
}

function stepBot(dt){
  if(!BOT.on||!S.running||S.doom||S.passedOut>0)return;
  BOT.acc+=dt; if(BOT.dwell>0)BOT.dwell-=dt; if(BOT.acc<100)return; BOT.acc=0;
  botRememberCurrentFeed();

  /* A Gordon impact has already been survived; release that fail-open door
     immediately. This is safe even with Eugene present because opening no
     longer requires the mains. Eugene remains the first threat action after it. */
  if(botHandleDoorRelease())return;
  /* Absolute threat priority: Eugene at the doorway means DARK NOW. */
  if(botForceDarkForEugene()) return;
  if(botHandleJeffrey())return;
  if(!S.sys.cam.ok){botHandleReboot("cam");return;}

  /* GORDON ALWAYS WINS A TIE.
     Missing one Sloppy check costs ground; missing a Gordon door can put him in
     an upstairs duct with no remaining counterplay. His tell therefore
     interrupts an in-progress Sloppy watch immediately, even at 4.9 seconds.
     The watch bank is persistent, so the bot resumes it after the Gordon job. */
  /* Gordon normally interrupts maintenance, but DO NOT throw away a nearly
     finished camera/door reboot that is itself required to answer him. If the
     remaining repair plus a conservative locate/door reserve still fits inside
     the audible warning, finishing it is strictly better than resetting six
     seconds of work to zero. This was a real benchmark failure: the door array
     had only ~1.1s left when Gordon warned, and the old bot cancelled it. */
  if(BOT.gordonAlert && rebootActive()){
    const active=botActiveRebootIds();
    const now=performance.now();
    const timeLeft=BOT.gordonDeadline>0 ? BOT.gordonDeadline-now : 0;
    const relevant=active.filter(id=>id==="cam"||id==="door");
    const remain=relevant.length ? Math.max(...relevant.map(id=>S.sys[id].reboot||0)) : Infinity;
    const reserve=1450; // close panel + snap search + floor switch + door command
    if(relevant.length && remain + reserve <= timeLeft){
      if(!S.panelOpen) togglePanel();
      return;
    }
    /* Vent/audio work is never worth sacrificing Gordon for. A ventilation
       reboot may be retried after the door event; Eugene still has absolute
       priority above this block. */
    cancelReboots();
  }
  if(botHandleGordon())return;

  /* A Sloppy doorway arrival is no longer a routine observation cycle. Give
     the emergency audio recall its chance before starting/continuing ordinary
     maintenance. Gordon's active four-second tell still outranks it above. */
  if(botHandleDoorwayThreats())return;

  /* While Gordon is still downstairs, a dead door array is more dangerous
     than one missed Sloppy observation cycle. Repair it proactively instead of
     waiting for the next four-second tell, because a six-second reboot cannot
     be completed after that warning has already begun. */
  if(!BOT.gordonUp && S.gordon && S.gordon.phase === "down" &&
     S.sys.door && !S.sys.door.ok && S.sys.door.reboot <= 0){
    const doorSteps=botGordonStepsToNearestDoor();
    const sloppy=S.units.find(u=>u.commitModel);
    const sloppyUrgent=BOT.sloppyNeeds && sloppy && !sloppy.breaching &&
                       (sloppy.watchProgress||0) < sloppyCfg().watchMs;
    /* If memory proves Gordon is still at least two rooms away from any live
       door, bank Sloppy first. One missed Sloppy cycle is recoverable, but four
       consecutive misses while repeatedly rebooting a door array are not. */
    if(!(sloppyUrgent && doorSteps >= 2)){
      botHandleReboot("door"); return;
    }
  }

  const reboots=botActiveRebootIds();
  const urgent=BOT.sloppyNeeds||S.units.some(u=>u.breaching);
  if(reboots.length){
    /* Once air is this low a ventilation reboot is no longer housekeeping.
       Sloppy can wait one cycle; oxygen cannot. Eugene still interrupts above. */
    const critical=reboots.includes("cam") ||
      (reboots.includes("door") && !BOT.gordonUp && S.gordon && S.gordon.phase === "down" &&
       botGordonStepsToNearestDoor() < 2) ||
      (reboots.includes("vent") && botVentCannotWaitForSloppy());
    if(urgent&&!critical){if(S.panelOpen)togglePanel();else cancelReboots();}
    else{if(!S.panelOpen)togglePanel();return;}
  }

  /* Sloppy goes first whenever the oxygen budget can pay for both jobs. If it
     cannot, ventilation preempts exactly as the player would when the air is
     no longer enough to finish five seconds plus a full reboot. */
  if(S.sys.vent && !S.sys.vent.ok && S.sys.vent.reboot<=0 && botVentCannotWaitForSloppy()){
    botHandleReboot("vent"); return;
  }
  if(botHandleSloppy())return;
  if(botHandleDowntimeFaults())return;
  botIdleDarkCycle();
}
...
function winNight(){
  ...
  if (secretNightMode()) saveSecretNightComplete();
  unlockNextNight();
  buildPickers();
  renderDeskRewards();
  document.body.classList.remove("secret-night");
  showVictory();
}
