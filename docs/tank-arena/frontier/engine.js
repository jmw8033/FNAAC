...
class Engine{
 ...
 const angle=p.angle;p.cool=it.cool;this.gather=null;this.bullets.push({x:p.x+Math.cos(angle)*31,y:p.y+Math.sin(angle)*31,vx:Math.cos(angle)*it.speed,vy:Math.sin(angle)*it.speed,life:2.5,player:true,owner:p.id,item:p.equipped,damage:D.rollDamage(it,this.level(it.skill),this.random),color:it.color});return true;}
 hitEnemy(e,damage,id){if(e.dead)return;const it=D.items[id],cfg=D.enemyTypes[e.type];const dealt=Math.min(e.hp,Math.max(1,damage-cfg.armor*(it.pierce?.5:1)));e.hp-=dealt;e.flash=.1;this.floaters.push({x:e.x,y:e.y-30,text:String(Math.round(dealt)),color:'#fff0ad',life:.8});this.xp(it.skill,dealt*.85);if(it.slow)e.slow=1.5;if(it.burn){e.burn=3;e.burnItem=id;e.burnOwner=this.player.id;}
 ...
 step(dt,input={}){if(this.paused)return;dt=clamp(dt,0,.05);this.time+=dt;this.stepPlayer(dt,input);this.stepWorld(dt);}
 stepPlayer(dt,input={}){const p=this.player;
 p.cool=Math.max(0,p.cool-dt);p.invuln=Math.max(0,p.invuln-dt);
 const throttle=clamp(input.throttle||0,-1,1),turn=clamp(input.turn||0,-1,1);
 global.FrontierMovement.drive(p,{...input,aim:input.aim?Math.atan2(input.aim.y-p.y,input.aim.x-p.x):p.angle},this.world,dt);
 ...
 stepWorld(dt){
 for(const e of this.enemies){const p=this.targetFor(e),cfg=D.enemyTypes[e.type];if(e.dead){if(this.time>=e.respawn&&distance(p,{x:e.homeX,y:e.homeY})>400){e.dead=false;e.x=e.homeX;e.y=e.homeY;e.hp=cfg.hp;e.burn=0;e.slow=0;}continue;}e.cool-=dt;e.slow=Math.max(0,e.slow-dt);e.flash=Math.max(0,(e.flash||0)-dt);
 if(e.burn>0){e.burn-=dt;e.burnTick-=dt;if(e.burnTick<=0){e.burnTick=1;e.hp-=4;this.forOwner(e.burnOwner,()=>this.xp('Rockets',3));this.floaters.push({x:e.x,y:e.y-30,text:'4',color:'#ffae77',life:.6});if(e.hp<=0){this.forOwner(e.burnOwner,()=>this.killEnemy(e));continue;}}}
 ...
 if(b.life<=0||this.world.blocked(b.x,b.y,3,true)){this.forOwner(b.owner,()=>this.impact(b,null));break;}
 if(b.player){const e=this.enemies.find(e=>!e.dead&&distance(e,b)<24);if(e)this.forOwner(b.owner,()=>this.impact(b,e));}else this.forPlayers(()=>{if(!b.dead&&distance(this.player,b)<23){b.dead=true;this.damagePlayer(b.damage);}});
 }}this.bullets=this.bullets.filter(b=>!b.dead);
 for(const b of this.blasts){b.life-=dt;if(b.warning&&b.life<=0){this.forPlayers(()=>{if(distance(this.player,b)<b.r)this.damagePlayer(b.damage);});b.warning=false;b.life=.35;b.color='#efb175';}}this.blasts=this.blasts.filter(b=>b.life>0);
 for(const f of this.floaters){f.y-=dt*25;f.life-=dt;}this.floaters=this.floaters.filter(f=>f.life>0);
 }
 targetFor(){return this.player;}
 forOwner(id,fn){return fn();}
 forPlayers(fn){return fn();}
